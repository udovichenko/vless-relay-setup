...
- **3X-UI v3.x** — устанавливается и пинится автоматически (relay управляется через REST API панели)
...
Возвращает exit code `1` если что-то сломано — удобно для cron:
...
Изменения видны в веб-панели после рефреша. CDN-инбаунд синкается автоматически. Если xray не поднялся после изменений — auto-rollback на исходное состояние.
...
> **Переход на v1.11.0 (с версий ≤ v1.10.x):** в v1.11.0 управление relay переведено на REST API 3X-UI v3.x, поэтому бинарник панели обновляется с v2.8.11 до v3.1.0. Этот переход **требует** флага `--upgrade` — без него новый код не найдёт API v3 и установка сломается. Существующие клиенты мигрируют автоматически (сидер 3X-UI переносит их в нормализованные таблицы). **Перед обновлением сделайте бэкап** `/etc/x-ui/x-ui.db`. Порядок прежний: сначала exit, затем relay.
